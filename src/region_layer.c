...
layer make_region_layer(int batch, int w, int h, int n, int classes, int coords)
{
    ...
    fprintf(stderr, "detection by region\n");
    srand(0);
...
void get_region_boxes_gpu(layer l, int w, int h, int netw, int neth, float thresh, float **probs, box *boxes, int only_objectness, int *map, float tree_thresh, int relative)
{
    int i,j,n;
    float *predictions = l.output;
    int wh = l.h * l.w;
    int coords_classes = l.coords + l.classes + 1;
    ...
    for (i = 0; i < wh; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            int obj_index = (nwh) * (coords_classes) + 4 * wh + (nwh + i) % (wh);
            int box_index = (nwh) * (coords_classes) + (nwh + i) % (wh);
            // printf("%d %d \n", box_index2, box_index);
            float scale = l.background ? 1 : predictions[obj_index];
            int stride = wh;
            box b;
            b.x = (col + predictions[box_index + 0 * stride]) / l.w;
            b.y = (row + predictions[box_index + 1 * stride]) / l.h;
            b.w = exp(predictions[box_index + 2 * stride]) * l.biases[2*n]   / l.w;
            b.h = exp(predictions[box_index + 3 * stride]) * l.biases[2*n+1] / l.h;
            boxes[index] = b;
            // printf("########### %f #########\n", l.biases[2 * n]);
            // boxes[index] = get_region_box(predictions, l.biases, n, box_index, col, row, l.w, l.h, l.w*l.h);
            // printf("##### %f %f %f %f\n", b.x, b.y, b.w, b.h);
            ...
            float max = 0;
            for(j = 0; j < l.classes; ++j){
                int class_index = entry_index(l, 0, nwh + i, 5 + j);
                float prob = scale*predictions[class_index];
                probs[index][j] = (prob > thresh) ? prob : 0;
                if(prob > max) max = prob;
            }
